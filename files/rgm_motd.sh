...
CPU_INFO="$(grep -c processor /proc/cpuinfo)x$(grep 'model name' /proc/cpuinfo | uniq | awk -F ':' '{print $2}')"
MEMORY="$(free -m |grep Mem: | awk -F " " '{print $2}') MB"
MEMORY_USED="$(free -m |grep Mem: | awk -F " " '{print $3}') MB"

echo -e "\n\e[1m -+- Welcome \e[34mRGM\e[39m server - https://www.rgm-cloud.io -+-\e[0m\n"
